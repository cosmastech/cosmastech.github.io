...
must the _The Way_. I don't like it, but I can admit it: PHP has a stink to it. It's not taken seriously.
...
for the right job. If I am mapping Eloquent Models to plain objects to pass to repositories to then use
a Model's query builder in the repository in order to upsert data, then mapping it into a different
domain object, what am I doing? A lot of work for idealism.
...
I am not here to make the claim that anyone else is doing it wrong. But I do think the benefit
...
Good testing is made possible through good code composition. There's nothing about Eloquent Models that make
this impossible. There are some guard rails to put in place (requiring data to be preloaded or passed as a
separate argument to a function, being careful about how data is mutated, never using Model Observers, using
bulk inserts whenever possible, rather than single updates in a loop), but the code can be just as testable
as it would be if written in a different paradigm.
...
1. Developers get in the habit of not running the test suite locally. They just leave it for CI.
2. I'll run the pipeline locally and try to multitask. What happens is that I think to myself "I'll just
go and work on something else for nine minutes." 30 minutes later, I remember to check the suite locally,
see there was a failure, and switch context back. The time I spend waiting for the pipeline is usually not
terribly productive: I'm either doing something non-work related (twitter, reddit, etc), or I'm half-reviewing
someone else's code.
  a. Oh, and then once my suite looks clean on local, I push my code and run the pipeline there, potentially repeating the process of waiting, getting lost, and paying a context switch.

Slow CI pipelines slow down merges to trunk and releases to production.
...
them devilish, but I would say they're worth actively avoiding for anything but a stub.
...
I strongly dislike how much code I have to review now. Within my editor reviewing an LLM's code, or the influx
of pull requests I have to review from other devs. Agents, specifically Cursor's BugBot, that automatically
review code is a good first step. However, I see a problem with all of this: my brain turns off. I stop
thinking about code the same way, or even really trusting my own instincts.
...
in fact, I think it's pretty standard in the industry. Once it becomes the norm, it's unbearable.
...
if the ticket is describing exactly what is SUPPOSED to be happening. I am forced to ask for help, which
