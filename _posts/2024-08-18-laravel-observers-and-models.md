...
Additionally, decomposing functionality makes it easier to write testable components.  I can focus on a single function and test that it does what it is intended to do.  If I only test observers indirectly, such as by calling an endpoint which updates the user and confirming a that job does, my tests are very brittle and are tied to implementation details.  The more setup required to test something, the less likely I find myself actually testing every branch of that indirect function.
